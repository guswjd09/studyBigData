...
serviceKey='hPk4OZJUYfsTJI9GGfvfuhOY9CftkP1wCUXNieRa1x1WcJSteY%2B6KFhnu8urIsBssmJaxK8gDmWKGB1u9FDSVQ%3D%3D'
        ...
        if jsonData['getgmgcourseinfo']['item'] == '':
            ...
            for item in jsonData['getgmgcourseinfo']['item']:
                ...
                result.append([seq, course_nm, gugan_nm, gm_range, gm_degree, 
                               start_pls, start_addr, middle_pls, middle_adr,
                               end_pls, end_addr, gm_course, gm_text])
...
def main():
    ...
    if len(result) > 0:
        # csv 파일저장
        columns = ['seq', 'course_nm', 'gugan_nm', 'gm_range', 'gm_degree', 
                   'start_pls', 'start_addr', 'middle_pls', 'middle_adr',
                   'end_pls', 'end_addr', 'gm_course', 'gm_text']
